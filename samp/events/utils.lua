...
function utils.decompress_weapon_and_special_key(v)
	return bit.band(v, 0x3F), bit.rshift(v, 6)
end

function utils.compress_weapon_and_special_key(weapon, specialKey)
	return bit.bor(bit.band(weapon, 0x3F), bit.lshift(bit.band(specialKey, 0x03), 6))
end
...
function utils.process_outgoing_sync_data(bs, st)
